...
- **Home offset (v1.2.0)** : `setHome()` capture l'attitude courante via `qHomeInverse = quatConj(q) * qYaw_h`. La formule isole la composante roll/pitch du mount (`quatConj(q) * qYaw_h = quatConj(q_mount_rp)`) et laisse le yaw absolu intact. **Requiert filterMode = MAHONY** (le complementary filter ne maintient pas un quaternion fiable). Persisté NVS, requiert bump `MPU9250_CALIB_VERSION` (passé v1 → v2 lors de l'introduction). Une `CalibBlob` v1 ancienne est rejetée → force recalibration au boot.
- **Publish bundle = orientation/quaternion CORRIGÉS par home** depuis v1.2.0. Le membre interne `q` (état Mahony) reste absolu — c'est la version chassis-frame qui part dans le `SampleBundle` et donc dans le `TelemetryDTO`. `getAbsoluteOrientation()` / `getAbsoluteQuaternion()` exposent l'absolu pour les rares cas (debug, recalcul mag heading précis).
- **Mahony fast-init boost (v1.2.0)** : `Config::mahonyBoostKp` (default 10) appliqué pendant `Config::mahonyBoostDurationMs` (default 3000) ms au démarrage de `sensorTask` ET après `performGyroAccelCalibration`. Implémentation : `mahonyBoostUntilUs` deadline, `updateMahonyFilter` substitue `effectiveKp = mahonyBoostKp` jusqu'à expiration puis loggue le retour. `triggerMahonyBoost()` re-arme la fenêtre (utile après collision/atterrissage brutal). `setMahonyBoost(kp, ms)` reconfigure runtime. `boostKp < mahonyKp` clamped (pas de sens de boost downward). `durationMs == 0` désactive le boost — `triggerMahonyBoost` devient silent no-op.
